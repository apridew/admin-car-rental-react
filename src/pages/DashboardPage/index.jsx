...
const DashboardPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    const lastDayOfMonth = new Date(selectedYear, new Date(`${selectedMonth} 1, ${selectedYear}`).getMonth() + 1, 0)
      .toISOString()
      .split("T")[0];

    try {
      const ress = await axios.get(`https://api-car-rental.binaracademy.org/admin/order/reports?from=${firstDayOfMonth}&until=${lastDayOfMonth}`, config)
      console.log(ress.data);


      const newLabels = Array.from({ length: ress.data.length }, (_, index) => index + 1);
  ...
  const columns = [
    {
      name: 'No',
      // selector: (row, index) => `${(table.page - 1) * table.pageSize + index + 1}`,
      selector: (row, index) => index + 1,
      sortable: true,
      sortFuntion: (rowA, rowB) => rowB - rowA,
  ...
  const handleButton = () => {
    console.log("clicked");
    // handleTable({ selected: 0 })
    handleTable({ selected: jumpToPage - 1 });
  }
              ...
              <div className='dashboard'>
                ...
                <div className="dashbboard-container">
                  ...
                  <div className="dashboard-month">
                    ...
                    <button onClick={handleSubmit} className='calendar-btn'>Go</button>
                  </div>
                ...
                <div className="table-container">
                  ...
                  <div className="dashboard-paginate">
                    <div className="page-limit">
                      <div className="limit">
                        <p>Limit</p>
                        <select name="limit" id="limit" onChange={handleChangeSelect} value={limit}>
                      ...
                      <div className="jump-to-page">
                        <p>Jump To Page</p>
                        <select name="jump-to-page" id="jump-to-page" onChange={handleJumpToPage} value={jumpToPage}>
                          {Array.from({ length: orders.pageCount }, (_, i) => i + 1).map((item, index) => (
